...
export default function Residents() {
  ...
  const handleDelete = async (r) => {
    if (!confirm(`Hapus penghuni "${r.name}"? Data tidak bisa dikembalikan.`))
      return;
    setDeleting(r.id);
    try {
  ...
  const filtered = residents.filter((r) => {
    ...
    const matchStatus =
      statusFilter === "all" || r.resident_type === statusFilter;
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            {filtered.length === residents.length
              ? `${residents.length} penghuni terdaftar`
              : `${filtered.length} dari ${residents.length} penghuni`}
          </p>
        </div>
          ...
          {STATUS_FILTERS.map((f) => (
            ...
            >
              ...
              >
                {f.value === "all"
                  ? residents.length
                  : residents.filter((r) => r.resident_type === f.value).length}
              </span>
            </button>
          ))}
        </div>
                ...
                <tr>
                  ...
                  >
                    {search || statusFilter !== "all"
                      ? "Tidak ada penghuni yang cocok dengan filter"
                      : "Tidak ada penghuni"}
                  </td>
                </tr>
              )}
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setForm({ ...form, resident_type: e.target.value })
                }
              >
          ...
          <div className="flex items-center gap-3">
            <input
              ...
              onChange={(e) =>
                setForm({ ...form, is_married: e.target.checked })
              }
          ...
          <div>
            ...
            {photoPreview && (
              <img
                src={photoPreview}
                alt="Preview KTP"
                className="mt-2 rounded-lg border max-h-32 object-contain"
              />
            )}
          </div>
